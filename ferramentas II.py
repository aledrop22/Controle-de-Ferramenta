...
if st.session_state.tela_atual == 'dashboard':
    ...
    with col_devolvidos:
        ...
        df_devolvidos = df[df['Status'] == 'Devolvido'].copy()
        df_devolvidos['Data/Horas - Retirada'] = df_devolvidos['Data_Retirada'] + ' às ' + df_devolvidos['Hora_Retirada']
        df_devolvidos['Data/Hora - Devolução'] = df_devolvidos.apply(
            lambda x: f"{x['Data_Retorno']} às {x['Hora_Retorno']}" if x['Data_Retorno'].strip() and x['Hora_Retorno'].strip() else 'Pendente',
            axis=1
        )
        ...
        df_display = df_devolvidos[['Instrumento', 'Especificacao', 'Operador', 'Maquina', 'Data/Horas - Retirada', 'Data/Hora - Devolução']]
        st.dataframe(df_display.fillna(''), hide_index=True, use_container_width=True)
        if df_devolvidos.empty:
