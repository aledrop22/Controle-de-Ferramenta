...
ARQUIVO_SQLITE = DIRETORIO_APP / 'registro_movimentacao_instrumentos.db'
ARQUIVO_CSV_LEGADO = DIRETORIO_APP / 'registro_movimentacao_instrumentos.csv'
COLUNAS_MOVIMENTACAO = ['ID', 'Instrumento', 'Especificacao', 'Operador', 'Setor', 'Maquina', 'Data_Retirada', 'Hora_Retirada', 'Data_Retorno', 'Hora_Retorno', 'Status', 'Finalizacao_Esquecimento']
...
def importar_csv_legado(conn):
    if not ARQUIVO_CSV_LEGADO.exists():
        return

    quantidade_atual = conn.execute('SELECT COUNT(*) FROM movimentacoes').fetchone()[0]
    if quantidade_atual > 0:
        return

    dados_legados = pd.read_csv(ARQUIVO_CSV_LEGADO, sep=';', encoding='utf-8-sig', dtype=str).fillna('')
    for coluna in COLUNAS_MOVIMENTACAO:
        if coluna not in dados_legados.columns:
            dados_legados[coluna] = ''

    conn.executemany('''
        INSERT OR IGNORE INTO movimentacoes
        (ID, Instrumento, Especificacao, Operador, Setor, Maquina,
         Data_Retirada, Hora_Retirada, Data_Retorno, Hora_Retorno,
         Status, Finalizacao_Esquecimento)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', dados_legados[COLUNAS_MOVIMENTACAO].itertuples(index=False, name=None))

def carregar_dados():
    inicializar_banco()
    conn = sqlite3.connect(ARQUIVO_SQLITE)
    importar_csv_legado(conn)
    conn.commit()
    df = pd.read_sql_query("SELECT * FROM movimentacoes", conn)
    conn.close()
    
    if df.empty:
        return pd.DataFrame(columns=COLUNAS_MOVIMENTACAO)
...
def salvar_dados(df):
    try:
        inicializar_banco()
        conn = sqlite3.connect(ARQUIVO_SQLITE)
        dados = df.reindex(columns=COLUNAS_MOVIMENTACAO).fillna('').astype(str)
        conn.executemany('''
            ...
            ON CONFLICT(ID) DO UPDATE SET
                ...
                Hora_Retirada=excluded.Hora_Retirada,
                Data_Retorno=excluded.Data_Retorno,
                Hora_Retorno=excluded.Hora_Retorno,
                Status=excluded.Status,
                Finalizacao_Esquecimento=excluded.Finalizacao_Esquecimento
            ...
            for (operador, setor, maquina), group in grouped:
                with st.container(border=True):
                    ...
                    with c2:
                        ...
                        # No modo chão de fábrica: mostrar botão de transferência
                        # No modo qualidade: mostrar botão de devolução individual
                        st.markdown("---")
    ...
    if not modo_chao_fabrica:
        ...
        with st.container(border=True):
            ...
            if not df_devolvidos.empty:
                ...
                with col1:
                    filtro_periodo = st.selectbox("Período:", ["Este Mês", "Hoje", "Últimos 7 dias", "Últimos 30 dias", "Últimos 60 dias", "Últimos 90 dias", "Mês Anterior", "Todos"], key="filtro_periodo")
                with col2:
                ...
                if filtro_periodo != "Todos":
                    ...
                    if filtro_periodo == "Hoje":
                        ...
                        df_filtrado = df_filtrado[
                            (df_filtrado['Data_Retorno_dt'] >= primeiro_dia_mes_anterior) &
                            (df_filtrado['Data_Retorno_dt'] <= ultimo_dia_mes_anterior)
                        ]
